...
- **`scripts/local-stack.ps1`** — publishes the Monitor + Web pair into a release directory, points the scheduled tasks at it, restarts them in a safe order (Web down, Monitor down, swap, Monitor up, Web up) and verifies the dashboard; `-Action status` inspects the running stack. Task ownership must be unambiguous (several tasks running the same executable abort the run), only processes from the stack's own release directories are stopped, a port that never frees refuses the cutover, and a failed action update automatically restores the tasks that were already re-pointed. See `docs/local-stack.md`.
